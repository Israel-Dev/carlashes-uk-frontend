...
const Home = () => {
    ...
    >([
        ...
        },
        {
            src: image02,
            alt: 'Anabelle',
            url: 'product?product_ref=1297jnasd87',
        },
    ...
    const getMosaicOptions = async () => {
        try {
            const response = await axios.get(
                `${REACT_APP_SERVER_ADDRESS}/resource/getMosaicOptions`
            );
    ...
    return (
        <Styles className="home-wrapper">
            <Carousel hasSticker={true} hasDots={true} />
            <MosaicContent />
            <Banner />

            <section className="collection-section">
                <Title text="Our Collection" />
                <Mosaic items={mosaicOptions} />
            </section>
            <section className="testimonials-section">
                <Title
                    text="Testimonials"
                    subtext="Some kind words from our clients"
                />
                <CardCarousel />
            </section>
